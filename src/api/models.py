...
class Profesional(db.Model):
    ...
    email = db.Column(db.String(100), unique=True, nullable=False)
    password = db.Column(db.String(100), nullable=False)
    id_oficio = db.Column(db.Integer, db.ForeignKey('oficio.id'),nullable=True)
...
class Tipo_consulta(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    id_oficio = db.Column(db.Integer, db.ForeignKey('oficio.id'),nullable=False)
...
class Oficio(db.Model):
    ...
    tipo_consulta = db.relationship('Tipo_consulta', backref='oficio', lazy=True)
    def __repr__(self):
...
class Consulta(db.Model):
    ...
    id_profesional = db.Column(db.Integer, db.ForeignKey('profesional.id'),nullable=False)
    id_tipo_consulta = db.Column(db.Integer, db.ForeignKey('tipo_consulta.id'),nullable=False)
